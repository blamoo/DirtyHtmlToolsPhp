...
class Sanitize
{
	...
	private static function filterTag(Tag $tag, array $rules)
	{
		...
		$tag->attributes = array_filter($tag->attributes, function ($val, $key) use ($rules, $tag) {
			if (!isset($rules[$tag->name]) || !is_array($rules[$tag->name])) {
				return false;
			}

			return in_array($key, $rules[$tag->name]);
